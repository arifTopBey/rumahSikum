        ...
        <div class="container" style="max-width: 900px; margin-top: 60px;">
            ...
            <div class="d-flex align-items-center justify-content-between mb-4">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <h4 class="fw-bold text-dark mb-0">{{ $modul->urutan ?? '1' }}.
                        {{ $modul->judul ?? 'Mengelola Keuangan Bisnis' }}</h4>
                </div>
            ...
            <ul class="nav nav-tabs border-0 gap-4 mb-4 custom-nav-tabs">
                <li class="nav-item">
                    <button class="nav-link active px-1 py-2 fw-bold" id="materi-tab" data-bs-toggle="tab"
                        data-bs-target="#materi-content" type="button">Materi</button>
                </li>
                <li class="nav-item">
                    <button class="nav-link px-1 py-2 fw-semibold text-muted" id="video-tab" data-bs-toggle="tab"
                        data-bs-target="#video-content" type="button">Video</button>
                </li>
                <li class="nav-item">
                    <button class="nav-link px-1 py-2 fw-semibold text-muted" id="quiz-tab" data-bs-toggle="tab"
                        data-bs-target="#quiz-content" type="button">Quiz</button>
                </li>
                <li class="nav-item">
                    <button class="nav-link px-1 py-2 fw-semibold text-muted" id="diskusi-tab" data-bs-toggle="tab"
                        data-bs-target="#diskusi-content" type="button">Diskusi</button>
                </li>
                <li class="nav-item">
                    <button class="nav-link px-1 py-2 fw-semibold text-muted" id="file-tab" data-bs-toggle="tab"
                        data-bs-target="#file-content" type="button">File</button>
                </li>
            </ul>
                            ...
                            <div class="d-flex align-items-center gap-3">
                                ...
                                <div>
                                    <h6 class="fw-bold text-dark mb-1 small">
                                        {{ $modul->judul ?? 'Mengelola Keuangan Bisnis' }}</h6>
                                    <span class="text-muted smaller">{{ $modul->tipe_materi ?? 'PPT' }} • <a
                                            href="{{ $modul->tautan ?? '#' }}" target="_blank"
                                            class="text-muted text-decoration-none">Buka materi</a></span>
                                </div>
                            </div>
                            <div>
                                <a href="{{ $modul->tautan ?? '#' }}" target="_blank" class="btn btn-link text-muted p-1"
            ...
            <div class="d-flex align-items-center justify-content-between mb-4">
                <div>
                    @if(isset($prevModul) && $prevModul)
                        <a href="{{ route('frontend.e-learning.detail.modul', [$modul->event_organizer_id, $prevModul->id]) }}"
                            class="btn btn-white bg-white border rounded-3 px-3 py-2 text-secondary fw-semibold small d-flex align-items-center gap-1 shadow-sm">
                ...
                <div>
                    @if(isset($nextModul) && $nextModul)
                        <a href="{{ route('frontend.e-learning.detail.modul', [$modul->event_organizer_id, $nextModul->id]) }}"
                            class="btn btn-white bg-white border rounded-3 px-3 py-2 text-secondary fw-semibold small d-flex align-items-center gap-1 shadow-sm">
            ...
            <form action="{{ route('frontend.e-learning.materi.selesai', [$modul->event_organizer_id, $modul->id]) }}"
                method="POST">
                @csrf

                @if (Auth::check() && $eventRegister)

                    <!-- <button type="sumbit" class="btn btn-success w-100 rounded-3 py-3 fw-bold d-flex align-items-center justify-content-center gap-2 shadow-sm" style="border: none;">
                    ...
                    @if ($progress && $progress->completed_at)
                        ...
                        <button type="button"
                            class="btn btn-success w-100 rounded-3 py-3 fw-bold d-flex align-items-center justify-content-center gap-2 shadow-sm"
                    ...
                    @else
                        ...
                        <button type="submit"
                            class="btn btn-primary w-100 rounded-3 py-3 fw-bold d-flex align-items-center justify-content-center gap-2 shadow-sm"
                ...
                @elseif(!Auth::check())
                    <a href="{{ route('login') }}"
                        class="btn btn-danger w-100 rounded-3 py-3 fw-bold d-flex align-items-center justify-content-center gap-2 shadow-sm"
                        style="border: none;">
                        <i data-lucide="check-circle-2" size="20"></i>
