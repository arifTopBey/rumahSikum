                        ...
                        <div class="detail-card shadow-sm">
                            <div class="d-flex align-items-center justify-content-between border-bottom pb-3 mb-3">
                                ...
                                <div class="text-end">
                                    ...
                                    <span
                                        class="small fw-bold text-dark">{{ $order->created_at->translatedFormat('d F Y, H:i') }}
                                        WIB</span>
                                </div>
                            </div>
                            ...
                            <div class="d-flex flex-column gap-3">
                                @foreach($order->details as $detail)
                                    <div
                                        class="d-flex align-items-center justify-content-between py-2 border-bottom {{ $loop->last ? 'border-0 pb-0' : '' }}">
                                        <div class="d-flex align-items-center gap-3">
                                            <img src="{{ route('show.thumbnail.produk.private', $detail->produk->produk_thumbnail) }}"
                                                class="product-img-detail border" alt="{{ $detail->produk->nama_produk }}">
                                            <div>
                                                <span
                                                    class="badge bg-light text-primary border fw-bold px-2 py-1 rounded-2 smaller mb-1 d-inline-block">
                                                    <i data-lucide="store" size="10" class="me-1"></i>
                                                    {{ $detail->vendor->nama_toko ?? 'UMKM' }}
                                                </span>
                                                <h6 class="fw-bold text-dark mb-1 text-truncate" style="max-width: 260px;">
                                                    {{ $detail->produk->nama_produk }}</h6>
                                                <span class="small text-muted">{{ $detail->qty }} x Rp
                        ...
                        <div class="detail-card shadow-sm">
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <span
                                        class="fw-bold text-dark text-uppercase small">{{ str_replace('_', ' ', $order->payment_method) }}</span>
                                </div>
                                <div class="col-12 pt-2">
                                    <span class="small text-muted d-block mb-1">Alamat Penerima</span>
                                    <div class="info-box">
                                        <h6 class="fw-bold text-dark mb-1 small">{{ $order->user->name ?? 'Pembeli' }}
                                            ({{ $order->user->no_hp ?? '-' }})</h6>
                                        <p class="text-muted small mb-0">
                                            {{ $order->user->alamat ?? 'Alamat pengiriman belum diisi.' }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="detail-card shadow-sm">
                            <div class="d-flex align-items-center justify-content-between mb-3">
                                ...
                                @if(!$order->bukti_pembayaran && $order->payment_status == 'pending')
                                    <a href="{{ route('frontend.payment.instruction', $order->invoice_number) }}"
                                        class="btn btn-sm btn-warning fw-bold rounded-pill px-3">
                                        Upload Bukti
                                    </a>
                                @endif
                            </div>
                            ...
                            @if($order->bukti_pembayaran)
                                <div class="d-flex align-items-center gap-3">
                                    <a href="{{ route('show.thumbnail.produk.private', $order->bukti_pembayaran) }}"
                                        target="_blank">
                                        <img src="{{ route('show.thumbnail.produk.private', $order->bukti_pembayaran) }}"
                                            class="rounded-3 border" style="width: 100px; height: 100px; object-fit: cover;">
                                    </a>
                                    <div>
                                        <span
                                            class="badge bg-success-subtle text-success border border-success fw-bold px-2 py-1 rounded-2 smaller mb-1">Sudah
                                            Diunggah</span>
                                        <p class="small text-muted mb-0">Klik pada gambar untuk melihat bukti transfer dalam
                                            ukuran penuh.</p>
                                    </div>
                                </div>
                            @else
                    ...
                    <div class="col-lg-5">
                        <div class="detail-card shadow-sm sticky-top" style="top: 110px;">
                            ...
                            <div class="d-flex justify-content-between align-items-center mb-4">
                                ...
                                <h5 class="fw-800 text-primary mb-0">Rp
                                    {{ number_format($order->total_payment, 0, ',', '.') }}</h5>
                            </div>
                            ...
                            <div class="d-grid gap-2">
                                @if($order->payment_status == 'pending')
                                    <a href="{{ route('frontend.payment.instruction', $order->invoice_number) }}"
                                        class="btn btn-warning rounded-pill py-2 fw-bold text-dark">
                                ...
                                @elseif($order->order_status == 'selesai')
                                    @php
                                        $totalProduk = $order->details->count();

                                        $totalReview = $order->review
                                            ->where('order_id', $order->id)
                                            ->count();
                                    @endphp

                                    @if($totalProduk > 0 && $totalReview >= $totalProduk)
                                        <button class="btn btn-outline-warning rounded-pill py-2 fw-bold" disabled>
                                            <i data-lucide="star-check" size="16" class="me-1"></i>
                                            Ulasan Sudah Diberikan
                                        </button>
                                    @else
                                        <a href="{{ route('frontend.ulasan', [$order->id, $order->details->first()->produk_id]) }}"
                                            class="btn btn-outline-warning rounded-pill py-2 fw-bold">
                                            <i data-lucide="star" size="16" class="me-1"></i>
                                            Beri Ulasan Produk
                                        </a>
                                    @endif
                                @endif
