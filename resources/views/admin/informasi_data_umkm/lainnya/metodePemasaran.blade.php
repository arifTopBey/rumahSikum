<div class="col-lg-12 px-2">
    <!-- <div class="d-flex justify-content-between py-2 ">
        <h4 id="detailTitle8" class="fw-bold text-primary mb-3 mt-5"></h4>
        <a href="{{ route('admin.export.gender', 'Laki-laki') }}" class="btn btn-success">
            Export Laki-laki
        </a>

        <a href="{{ route('admin.export.gender', 'Perempuan') }}" class="btn btn-success">
            Export Perempuan
        </a>

        <a href="{{ route('admin.export.gender', 'Tidak Diketahui') }}" class="btn btn-success">
            Export Tidak Diketahui
        </a>
    </div> -->
        ...
        <table class="table table-hover align-middle">
            <thead class=" text-white">
                <tr class="">
                    ...
                    <th style="background-color: rgba(108, 117, 125, 0.1);" class="fw-semibold text-dark ">Kecamatan
                    </th>
                </tr>
            </thead>
            <tbody>
                @foreach ($data as $umkm)
                    <tr>
                        <td class="text-center"><a href="{{ route('admin.umkm.detail', $umkm->id_badan_usaha) }}"
                                class="fs-3 text-dark text-decoration-none">:</a></td>
                        ...
                        <td class="text-center">
                            {{ preg_replace('/^[0-9.]+\s+/', '', $umkm->identitasUsaha->provinsi ?? '') }}
                        </td>
                        <td class="text-center">
                            {{ preg_replace('/^[0-9.]+\s+/', '', $umkm->identitasUsaha->kabupaten ?? '') }}
                        </td>
                        <td class="text-center">
                            {{ preg_replace('/^[0-9.]+\s+/', '', $umkm->identitasUsaha->kecamatan ?? '') }}
                        </td>
                    </tr>
                @endforeach
            </tbody>
