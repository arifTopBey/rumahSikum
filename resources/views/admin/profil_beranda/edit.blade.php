    ...
    <div class="row pt-5 mb-4">
        <div class="col-md-10 mx-auto d-flex justify-content-between align-items-center">
            <div>
                ...
                <p class="text-muted small mb-0">Perbarui sumber video profil (YouTube atau Video Lokal) untuk beranda utama.</p>
            </div>
            <div>
    ...
    <div class="row">
        ...
        @endif
            <div class="card border-0 shadow-sm rounded-4 p-4 bg-white mb-4">
                <form action="{{ route('admin.profil-beranda.update', $video->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4 bg-light p-3 rounded-3 border">
                        ...
                        <div class="d-flex gap-4">
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="status" id="active_youtube" value="1" 
                                    {{ (old('status', $video->status) == 1) ? 'checked' : '' }}>
                            ...
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="status" id="active_local" value="0" 
                                    {{ (old('status', $video->status) == 0) ? 'checked' : '' }}>
                            ...
                            <div class="mb-4">
                                ...
                                <div class="input-group border-2 rounded-3 overflow-hidden bg-light">
                                    ...
                                    <input type="text" id="youtube_url" name="video_youtube" class="form-control border-0 bg-light py-2 small" 
                                           value="{{ old('video_youtube', $video->video_youtube) }}" placeholder="Contoh: https://www.youtube.com/watch?v=xxxxxx" autocomplete="off">
                                </div>
                                <div class="form-text smaller text-muted mt-1">Perbarui jika ingin mengganti video YouTube.</div>
                            </div>

                            <div class="mb-4">
                                <label class="small fw-bold text-muted text-uppercase mb-2 d-block">Unggah Video Lokal Baru (Opsional)</label>
                                ...
                                <div class="form-text smaller text-muted mt-1">Biarkan kosong jika tidak ingin mengganti file video lokal saat ini Format wajib: MP4 Maksimal Ukuran 20MB.</div>
                            </div>
                            ...
                            <div id="preview_local_container" class="mb-4 d-none">
                                ...
                                <div class="ratio ratio-16x9 rounded-4 overflow-hidden border shadow-sm bg-dark">
                                    <video id="local_video_player" controls muted class="w-100 h-100">
                                        <source src="{{ $video->video_local ? asset('storage/' . $video->video_local) : '' }}" type="video/mp4">
                                        Browser Anda tidak mendukung pratinjau video.
                                    </video>
                                </div>
                            </div>

                            <div id="preview_placeholder" class="d-flex flex-column align-items-center justify-content-center border border-dashed rounded-4 p-5 text-muted d-none" style="height: 220px;">
                    ...
                    <div class="d-flex justify-content-end gap-2">
                        ...
                        <button type="submit" class="btn btn-primary rounded-3 fw-bold px-4 py-2 small shadow-sm">
                            <i data-lucide="save" size="16" class="me-1"></i> Perbarui Video Profil
                        </button>
                    </div>
                </form>
            </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        // Menyimpan data awal database untuk fitur 'Kembalikan Semula'
        const initialYoutubeUrl = youtubeInput.value;
        const initialLocalVideoSrc = localVideoPlayer.querySelector('source').src;

        // Mengatur visibility placeholder utama jika kedua preview kosong
        ...
        // --- HANDLING LOGIKA LOCAL VIDEO ---
        function updateLocalPreview() {
            // Periksa apakah ada file baru yang diunggah, jika tidak pakai file lama dari database
            if (localInput.files && localInput.files[0]) {
                const fileURL = URL.createObjectURL(localInput.files[0]);
                localVideoPlayer.src = fileURL;
                previewLocalContainer.classList.remove('d-none');
            } else if (initialLocalVideoSrc && !initialLocalVideoSrc.endsWith('/')) {
                // Jika file lama dari database ada jalankan preview bawaan
                localVideoPlayer.src = initialLocalVideoSrc;
                previewLocalContainer.classList.remove('d-none');
            } else {
        ...
        localInput.addEventListener('change', updateLocalPreview);


        // --- INSIALISASI PREVIEW SAAT HALAMAN PERTAMA DIBUKA ---
        updateYoutubePreview();
        updateLocalPreview();


        // --- TOMBOL KEMBALIKAN SEMULA ---
        btnCancel.addEventListener('click', function() {
            youtubeInput.value = initialYoutubeUrl;
            localInput.value = ''; // Reset pilihan file baru
            localVideoPlayer.src = initialLocalVideoSrc;
